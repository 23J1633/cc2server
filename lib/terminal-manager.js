...
/** A small process-backed terminal bridge used when node-pty is unavailable. */
export class TerminalManager extends EventEmitter {
  ...
  open(params = {}) {
    ...
    const output = (chunk) => {
      ...
      item.replay = (item.replay + data).slice(-MAX_REPLAY_BYTES)
  ...
  attach(terminalId) {
    const item = this.get(terminalId)
    if (!item) throw new Error('终端不存在或已退出')
    return { terminalId: item.terminalId, cwd: item.cwd, cols: item.cols, rows: item.rows, replay: item.replay, lastSeenAt: item.lastSeenAt }
  }
  ...
  keepAlive(terminalId) {
    ...
    if (!item) throw new Error('终端不存在或已退出')
    item.lastSeenAt = Date.now()
    return { terminalId: item.terminalId, lastSeenAt: item.lastSeenAt }
  }
  ...
  write(terminalId, data) {
    ...
    item.lastSeenAt = Date.now()
    item.child.stdin.write(value)
    return { terminalId: item.terminalId, bytes: Buffer.byteLength(value) }
  }

  resize(terminalId, cols, rows) {
    const item = this.get(terminalId)
    if (!item) throw new Error('终端不存在或已退出')
    item.cols = Math.max(1, Number(cols) || item.cols)
    item.rows = Math.max(1, Number(rows) || item.rows)
    return { terminalId: item.terminalId, cols: item.cols, rows: item.rows }
  }

  close(terminalId) {
    const item = this.get(terminalId)
    if (!item) return { terminalId: String(terminalId), closed: false }
    this.items.delete(item.terminalId)
    try { item.child.kill() } catch { /* already exited */ }
    return { terminalId: item.terminalId, closed: true }
  }

  closeAll() {
    for (const terminalId of [...this.items.keys()]) this.close(terminalId)
  ...
  sweep() {
    ...
    for (const item of [...this.items.values()]) {
      if (item.lastSeenAt < cutoff) this.close(item.terminalId)
    }
  }
}
